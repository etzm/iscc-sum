...
def test_recursive_option():
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--recursive", "--help"])
    assert result.exit_code == 0
    assert "Process directories recursively" in result.output


def test_no_recursive_option():
    # type: () -> None
    """Test that --no-recursive option is accepted."""
    runner = CliRunner()
    result = runner.invoke(cli, ["--no-recursive", "--help"])
    assert result.exit_code == 0
    assert "Process only files in the specified directory" in result.output
...
def test_exclude_option():
    ...
    # Test single exclude
    result = runner.invoke(cli, ["--exclude", "*.tmp", "--help"])
    assert result.exit_code == 0

    # Test multiple excludes
    result = runner.invoke(cli, ["--exclude", "*.tmp", "--exclude", "*.bak", "--help"])
    assert result.exit_code == 0
    assert "Exclude files matching the given glob pattern" in result.output


def test_max_depth_option():
    # type: () -> None
    """Test that --max-depth option is accepted."""
    runner = CliRunner()

    # Test with valid integer
    result = runner.invoke(cli, ["--max-depth", "2", "--help"])
    assert result.exit_code == 0
    assert "Maximum directory depth to traverse" in result.output

    # Test with invalid value (should fail)
    result = runner.invoke(cli, ["--max-depth", "invalid"])
    assert result.exit_code == 2
    assert "Invalid value" in result.output or "invalid literal" in result.output


def test_recursive_and_no_recursive_conflict():
    # type: () -> None
    """Test that --recursive and --no-recursive cannot be used together."""
    runner = CliRunner()

    # Create a test file to avoid "no files" error
    with runner.isolated_filesystem():
        with open("test.txt", "w") as f:
            f.write("test")

        result = runner.invoke(cli, ["--recursive", "--no-recursive", "test.txt"])
        assert result.exit_code == 2
        assert "iscc-sum: --recursive and --no-recursive cannot be used together" in result.output
